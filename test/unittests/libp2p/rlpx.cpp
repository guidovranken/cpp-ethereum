...
struct RLPXTestFixture: public TestOutputHelperFixture {
	RLPXTestFixture() : s_cppethsecp256k1(crypto::Secp256k1PP::get()) {}
	~RLPXTestFixture() {}

	crypto::Secp256k1PP* s_cppethsecp256k1;
};
...
BOOST_AUTO_TEST_CASE(test_secrets_from_go)
{
	...
	BOOST_REQUIRE(s_cppethsecp256k1->decryptECIES(recv.secret(), authPlain));
	bytes ackPlain = ackCipher;
	BOOST_REQUIRE(s_cppethsecp256k1->decryptECIES(init.secret(), ackPlain));
...
BOOST_AUTO_TEST_CASE(ecies_interop_test_primitives)
{
	...
	BOOST_REQUIRE(s_cppethsecp256k1->decryptECIES(kmK.secret(), kmPlain));
	...
	BOOST_REQUIRE(s_cppethsecp256k1->decryptECIES(kenc.secret(), plainTest1));
	...
	BOOST_REQUIRE(s_cppethsecp256k1->decryptECIES(kenc.secret(), plainTest2));
	...
	BOOST_REQUIRE(s_cppethsecp256k1->decryptECIES(kenc.secret(), plainTest3));
