...
#ifdef VERIFY
static void cppethsecp256k1_num_sanity(const cppethsecp256k1_num *a) {
    VERIFY_CHECK(a->limbs == 1 || (a->limbs > 1 && a->data[a->limbs-1] != 0));
}
#else
#define cppethsecp256k1_num_sanity(a) do { } while(0)
#endif
...
static void cppethsecp256k1_num_add_abs(cppethsecp256k1_num *r, const cppethsecp256k1_num *a, const cppethsecp256k1_num *b) {
...
static void cppethsecp256k1_num_sub_abs(cppethsecp256k1_num *r, const cppethsecp256k1_num *a, const cppethsecp256k1_num *b) {
...
static void cppethsecp256k1_num_mod(cppethsecp256k1_num *r, const cppethsecp256k1_num *m) {
    cppethsecp256k1_num_sanity(r);
    cppethsecp256k1_num_sanity(m);
    ...
    if (r->neg && (r->limbs > 1 || r->data[0] != 0)) {
        cppethsecp256k1_num_sub_abs(r, m, r);
        r->neg = 0;
    }
}
...
static int cppethsecp256k1_num_eq(const cppethsecp256k1_num *a, const cppethsecp256k1_num *b) {
    ...
    if ((a->neg && !cppethsecp256k1_num_is_zero(a)) != (b->neg && !cppethsecp256k1_num_is_zero(b))) {
        return 0;
    }
    return mpn_cmp(a->data, b->data, a->limbs) == 0;
}

static void cppethsecp256k1_num_subadd(cppethsecp256k1_num *r, const cppethsecp256k1_num *a, const cppethsecp256k1_num *b, int bneg) {
    if (!(b->neg ^ bneg ^ a->neg)) { /* a and b have the same sign */
        r->neg = a->neg;
        if (a->limbs >= b->limbs) {
            cppethsecp256k1_num_add_abs(r, a, b);
        } else {
            cppethsecp256k1_num_add_abs(r, b, a);
        }
    } else {
        if (cppethsecp256k1_num_cmp(a, b) > 0) {
            r->neg = a->neg;
            cppethsecp256k1_num_sub_abs(r, a, b);
        } else {
            r->neg = b->neg ^ bneg;
            cppethsecp256k1_num_sub_abs(r, b, a);
        }
    }
}
